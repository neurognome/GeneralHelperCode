    ...
    methods
      function obj = StepperController(motors)
		...
		function output = queue(obj, speed, input_type, value)
            ...
            speed = speed/min(speed); % Adjusting RPMs to scale properly?
            ...
            case 'steps'
                ...
                for n = 1:length(obj.motors)
                    n_steps(n) = (value(n) .* obj.aux_controller(n).getMicrostepScale() * speed(n));
                end
                duration = obj.getDuration(n_steps(1), speed(1));
            case 'seconds'
                duration = value;
                for n = 1:length(obj.motors)
                    n_steps(n) = obj.getSteps(duration, speed(n) * obj.aux_controller(n).getMicrostepScale());
                end
            end
